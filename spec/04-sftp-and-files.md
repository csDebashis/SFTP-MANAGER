...
- The browser first creates a durable upload session, then sends the file as bounded raw binary chunks. For a real SFTP server, the backend streams each request body directly to a uniquely named temporary object in the target SFTP folder without staging the complete file on the frontend or backend filesystem. The Vercel demo MOCK adapter persists each acknowledged chunk as a private Blob object and assembles those objects with Blob multipart upload before atomically publishing the destination key; no demo file depends on function-local `/tmp` storage.
